...
| 13.16 | Image size and the Italian model | §9.2 sized the image around the English model. The Italian model is an xlsr-53 large or xls-r 300M export, which is several times larger | **DECIDED 2026-09-13.** One image for every language. English is baked in, so the container works offline as soon as it starts. `MUSICBUTLER_LANGS` names the languages to run, and the entrypoint downloads the missing ones into the `/models` volume on the first start. `sync.languages` reports what is actually present, so it stays honest. This replaced the `WITH_IT_MODEL` build argument and the `latest-multilang` tag |
